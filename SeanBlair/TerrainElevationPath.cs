...
namespace SeanBlair
{
  ...
  public class TerrainElevationPath
  {
    ...
    // Requires latLonsInSameTile to have at least one element.
    ...
    private IEnumerable<int> ReadElevations(string fileName, List<LatLon> latLonsInSameTile)
    {
      ...
      using (BinaryReader binReader = new BinaryReader(File.Open(fileName, FileMode.Open)))
      {
        ...
        {
          string errorMessage = $"Error: The file with name {fileName} does not contain" +
              $" the required {HGTFileSize} bytes.";
    ...
    // Returns the number of bytes between the beginning of an .hgt file and the elevation
